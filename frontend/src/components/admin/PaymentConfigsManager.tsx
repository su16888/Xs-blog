...
export default function PaymentConfigsManager() {
  ...
  const handleUploadLogo = async (file: File) => {
    ...
    setError('')
    try {
      const res = await uploadAdminPaymentConfigLogo(editing.id, file)
      if (res?.success) {
        const url = res?.data?.display_logo || null
        setForm(prev => ({ ...prev, display_logo: url || '' } as any))
        await load()
      } else {
          ...
          <div className="lg:col-span-3 bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <div className="text-xs text-gray-600 mb-1">上传Logo（本地）</div>
                    <input
                      type="file"
                      accept=".jpg,.jpeg,.png,.gif,.webp,.svg,.ico"
                      disabled={!editing || logoUploading}
                      onChange={(e) => {
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {editing ? (logoUploading ? '上传中...' : '支持 jpg/jpeg/png/gif/webp/svg/ico；上传后会保存为 /uploads/... 相对路径') : '请先保存后再上传'}
                    </div>
                  </div>
                  <div className="flex items-end">
                    {(form as any).display_logo ? (
                      <div className="flex items-center gap-2">
                        <img
                          src={getImageUrl((form as any).display_logo)}
                          alt="logo"
                          className="w-10 h-10 rounded-lg border border-gray-200 object-cover"
                        />
                        <div className="text-xs text-gray-500 break-all">{String((form as any).display_logo)}</div>
                      </div>
                    ) : (
                      <div className="text-xs text-gray-400">暂无Logo</div>
                    )}
                  </div>
                </div>
              ...
              <div className="text-xs text-gray-500">
                  敏感字段会在后端使用 DB_ENCRYPTION_KEY 加密入库。
              </div>
            </div>
          </div>
        </div>
