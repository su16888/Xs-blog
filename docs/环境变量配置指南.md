...
Xs-Blog 支持两种数据库类型：MySQL 和 SQLite

| 变量名 | 说明 | 默认值 | 是否必填 |
|--------|------|--------|----------|
| `DB_DIALECT` | 数据库类型（mysql 或 sqlite） | mysql | ❌ |

##### MySQL 配置（当 DB_DIALECT=mysql 时）
...
##### SQLite 配置（当 DB_DIALECT=sqlite 时）

| 变量名 | 说明 | 默认值 | 是否必填 |
|--------|------|--------|----------|
| `DB_STORAGE` | SQLite 数据库文件路径 | ./database/xsblog.db | ❌ |
